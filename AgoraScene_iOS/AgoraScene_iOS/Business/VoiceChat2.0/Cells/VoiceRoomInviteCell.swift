...
public class VoiceRoomInviteCell: UITableViewCell {
    ...
    lazy var operation: UIButton = {
        UIButton(type: .custom).frame(CGRect(x: self.contentView.frame.width-80, y: self.avatar.center.y-15, width: 65, height: 30)).title("test", .normal).font(.systemFont(ofSize: 14, weight: .regular)).textColor(.white, .normal).setGradient([UIColor(0x219BFF),UIColor(0x345DFF)], [CGPoint(x: 0, y: 0),CGPoint(x: 0, y: 1)]).cornerRadius(15)
    }()
