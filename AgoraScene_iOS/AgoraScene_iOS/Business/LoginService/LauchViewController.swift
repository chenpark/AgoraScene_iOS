...
extension LauchViewController {
    ...
    @objc func login() {
        ProgressHUD.show()
        VoiceRoomBusinessRequest.shared.sendPOSTRequest(api: .login(()), params: ["deviceId":UIDevice.current.deviceUUID,"portrait":"avatar1","name":"chenpark"],classType:VRUser.self) { [weak self] user, error in
            ProgressHUD.dismiss()
            if error == nil {
