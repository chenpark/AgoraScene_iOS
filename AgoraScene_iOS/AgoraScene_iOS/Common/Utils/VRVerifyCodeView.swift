...
public class VRVerifyCodeView: UIView {
    ...
    func initSubviews() {
        // 每个验证码框宽度
        let itemWidth: CGFloat = CGFloat ((self.frame.width - padding * 2 - spacing * (CGFloat(inputTextNum) - 1))) / CGFloat(inputTextNum)
        for i in 0..<inputTextNum {
            let codeNumView = VRVerifyCodeNumberView(frame: CGRect(x: padding + CGFloat(i) * (spacing + itemWidth), y: 0, width: itemWidth, height: self.frame.height)).isUserInteractionEnabled(false).backgroundColor(.white).cornerRadius(8).layerProperties(UIColor(0xE4E3ED), 1)
            codeNumView.setCursorStatus(true)
            self.codeViews.append(codeNumView)
        }
        self.addSubViews(self.codeViews)
    }
...
public extension VRVerifyCodeView {
    ...
    func cleanCodes() {
        self.textFiled.text = ""
        self.textFiledDidChange(textFiled)
        self.allCursorHidden()
    }
